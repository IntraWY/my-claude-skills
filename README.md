...
| 2 | **sync-verify-deploy** | ตรวจสอบ/sync/deploy โปรเจกต์ใดก็ได้ (auto-detect) ข้าม local ↔ git remote(s) ↔ live — GAS/Apps Script (clasp) หรือ static site บน Cloudflare Pages (wrangler) | `sync ให้ชัวร์`, `เช็คว่า deploy ตรงไหน`, `local ตรงกับที่ deploy หรือยัง` | [`.skill`](./sync-verify-deploy.skill) |
